// Serviço completo de validação PIX
// Implementa todas as regras: favorecido, valor mínimo, data, ID único
...
const checkTransactionId = async (transactionId) => {
  try {
    ...
    if (response.ok) {
      ...
      return data.length === 0; // True se NÃO existir
    }
  } catch (error) {
...
export const validatePaymentReceipt = async (ocrText) => {
  ...
  details.date = timeDiff <= 5 * 60 * 1000; // 5 minutos em milissegundos
  if (!details.date) {
...
export const cleanOldTransactions = () => {
  ...
  localStorage.setItem('pix_transactions', JSON.stringify(filtered));
