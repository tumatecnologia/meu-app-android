...
export default function InterpretationDisplay({ cards, theme, personName, birthDate, question }) {
  ...
  // FUNÇÃO CORRIGIDA: Busca o texto detalhado do tema específico
  const getCardInterpretation = (card) => {
    ...
    // Normaliza o tema para buscar no objeto 'temas' do tarotData.js
    const activeTheme = theme 
      ? theme.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "") 
      : 'geral'; // Define um tema padrão se vier vazio
    
    // Busca: Detalhes do tema > Texto Upright > Texto geral
    return cardData.temas?.[activeTheme] || cardData.upright || "Interpretando as energias...";
  };

  return (
    <div className="mt-12 w-full max-w-4xl mx-auto mb-20 space-y-8 animate-fadeIn">
      
      {/* 1. DADOS DA CONSULTA - CORRIGIDO PARA MOSTRAR A PERGUNTA */}
      <div className="bg-white rounded-[2rem] p-8 shadow-xl border-l-8 border-purple-600">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6 text-gray-800">
          ...
          {/* MOSTRANDO A PERGUNTA CORRETA AQUI */}
          <p className="md:col-span-2"><strong className="text-purple-700">Pergunta:</strong> {question || 'Não formulada'}</p>
        </div>
      </div>

      {/* 2. ANÁLISE DO ORÁCULO - CORRIGIDO PARA MOSTRAR TEXTO DETALHADO */}
          ...
          {cards.map((card, index) => (
            <div key={index} className="prose prose-xl max-w-none text-gray-900 leading-relaxed border-b border-gray-100 pb-8 last:border-0">
              <h3 className="text-amber-600 font-bold uppercase text-sm mb-2 tracking-widest">
                ...
                {card.reversed && <span className="text-red-500 ml-2">(Invertida)</span>}
              </h3>
              {/* RENDERIZAÇÃO DO TEXTO DETALHADO AQUI */}
      ...
      <div className="bg-gray-900 rounded-[2rem] p-8 md:p-10 shadow-2xl text-white border-b-8 border-amber-500">
        ...
        <div className="prose prose-invert prose-lg max-w-none text-gray-200 text-center italic">
          <p>
            Analisando o tema <strong>{theme}</strong> sobre a questão "<em>{question}</em>", o oráculo sugere foco no momento presente. A combinação das cartas aponta para a necessidade de ação e confiança nas próximas etapas.
          </p>
        </div>
      </div>
    </div>
